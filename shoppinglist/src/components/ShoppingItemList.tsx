...
import { getItems, deleteItem } from "../api/shppingapi";
...
function ShoppingItemList(){
  ...
  const [snackbarMsg, setOpenSnackbarMsg] = useState('');
  ...
  const { mutate: deleteMutate} = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpenSnackbarMsg('해당 쇼핑 품목이 정상적으로 삭제되었습니다.');
  ...
  const columnDefs: ColDef<ShoppingItem>[] = [
    {field: '', sortable: true, filter: true, flex: 2},
    {field: '', sortable: true, filter: true, flex: 1},
    {
      field: 'purchased',
      sortable: true,
      filter: true,
  ...
  if(isError) {
    return <span>항목을 가져오는 데 오류가 발생습니다. : {error.message} </span>
  }
