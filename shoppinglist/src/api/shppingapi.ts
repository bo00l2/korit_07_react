...
import { AccountCredentials, ShoppingItem, ShoppngItemEntry } from '../types'
...
export const addItem = async (item: ShoppngItemEntry) : Promise<ShoppingItem> => {
  const response = await apiClient.post('/api/items', item, getRequestconfig());
  return response.data;
}

export const updateItem = async (id: number, itemUpdate: ShoppngItemEntry): Promise<ShoppingItem> => {
...
export const authenticateWithGoogleToken = async (idToken: string): Promise<string> => {
  console.log('Google ID 토큰을 백엔드로 전송한다.', idToken.substring(0, 30) + '...'); //  토큰 전체 노출 안하기 위해
  try {
    // 백엔드에 구현한 Google 토큰 검증 엔드포인트를 호출
    ...
    if(!backendJwt) {
      throw new Error('백엔드에서 JWT 토큰을 보내지 않는다.')
    }
